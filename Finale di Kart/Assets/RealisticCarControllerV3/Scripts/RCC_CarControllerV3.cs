...
public class RCC_CarControllerV3 : RCC_Core {
    ...
    [Obsolete("Deprecated and ineffective. Use overrideAllWheels.")]
    public bool overrideWheels = false;
    ...
    public bool
        autoGenerateEngineRPMCurve =
            true; // Auto create engine torque curve. If min/max engine rpm, engine torque, max engine torque at rpm, or top speed has been changed at runtime, it will generate new curve with them.
    ...
    public bool runEngineAtAwake {
        get { return RCC_Settings.Instance.runEngineAtAwake; }
    ...
    public bool autoReverse {
        get {
            if (!externalController) return RCC_Settings.Instance.autoReverse;
            else return true;
        }
    ...
    public bool automaticGear {
        get {
            if (!externalController) return RCC_Settings.Instance.useAutomaticGear;
            else return true;
        }
    ...
    public bool useAutomaticClutch {
        get { return RCC_Settings.Instance.useAutomaticClutch; }
    }
    ...
    public GameObject contactSparkle {
        get { return RCC_Settings.Instance.contactParticles; }
    } // Contact Particles for collisions. It must be Particle System.

    public GameObject scratchSparkle {
        get { return RCC_Settings.Instance.scratchParticles; }
    } // Scratch Particles for collisions. It must be Particle System.
    ...
    public delegate void carSpawned(RCC_CarControllerV3 rcc, bool isOwned);

    public static event carSpawned OnCarSpawned;
    ...
    void Awake() {
        ...
        if (steerAngleCurve == null)
            steerAngleCurve =
                new AnimationCurve(new Keyframe(0f, 40f), new Keyframe(120f, 7f),
                    new Keyframe(200f, 5f)); //	Steering angle limiter curve based on speed.
    ...
    private IEnumerator RCCPlayerSpawned() {
        ...
        if (!externalController) {
            ...
            OnRCCPlayerSpawned?.Invoke(this);
            OnCarSpawned?.Invoke(this, IsOwner);
    ...
    private void CreateAudios() {
        ...
        switch (audioType) {
            ...
            case AudioType.OneSource:
                ...
                engineSoundHigh = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Sound High AudioSource",
                    5, 50, 0, engineClipHigh, true, true, false);

                if (autoCreateEngineOffSounds) {

                    engineSoundHighOff = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition,
            ...
            case AudioType.TwoSource:
                ...
                engineSoundHigh = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Sound High AudioSource",
                    5, 50, 0, engineClipHigh, true, true, false);
                engineSoundLow = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Sound Low AudioSource", 5,
                    25, 0, engineClipLow, true, true, false);

                if (autoCreateEngineOffSounds) {

                    engineSoundHighOff = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition,
                        "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
                    engineSoundLowOff = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition,
            ...
            case AudioType.ThreeSource:
                ...
                engineSoundHigh = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Sound High AudioSource",
                    5, 50, 0, engineClipHigh, true, true, false);
                engineSoundMed = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Sound Medium AudioSource",
                    5, 50, 0, engineClipMed, true, true, false);
                engineSoundLow = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Sound Low AudioSource", 5,
                    25, 0, engineClipLow, true, true, false);
                ...
                if (autoCreateEngineOffSounds) {
                    ...
                    engineSoundHighOff = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition,
                        "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
                    engineSoundMedOff = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition,
        ...
        engineSoundIdle = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Sound Idle AudioSource", 5, 25, 0,
            engineClipIdle, true, true, false);
        reversingSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, gearSoundPosition, "Reverse Sound AudioSource", 10, 50, 0,
            reversingClip, true, false, false);
        windSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, windSoundPosition, "Wind Sound AudioSource", 1, 10, 0, windClip,
            true, true, false);
        ...
        if (useNOS)
            NOSSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, exhaustSoundPosition, "NOS Sound AudioSource", 5, 10, .5f,
                NOSClip, true, false, false);

        if (useNOS || useTurbo)
            blowSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, exhaustSoundPosition, "NOS Blow", 1f, 10f, .5f, null, false,
                false, false);

        if (useTurbo) {

            turboSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, turboSoundPosition, "Turbo Sound AudioSource", .1f, .5f, 0f,
                turboClip, true, true, false);
    ...
    public void InitGears() {
        ...
        for (int i = 0; i < gears.Length; i++) {
            ...
            targetSpeedForGear[i] =
                (int)(Mathf.Lerp(0, maxspeed * Mathf.Lerp(0f, 1f, gearShiftingThreshold), ((float)(i + 1) / (float)(gears.Length))));
    ...
    public IEnumerator StartEngineDelayed() {
        ...
        if (!engineRunning) {
            ...
            engineStartSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, engineSoundPosition, "Engine Start AudioSource", 1, 10, 1,
                engineStartClip, false, true, true);
            ...
            switch (steeringWheelRotateAround) {
                ...
                case SteeringWheelRotateAround.XAxis:
                    SteeringWheel.transform.localRotation = orgSteeringWheelRot *
                                                            Quaternion.AngleAxis(((steerInput * steerAngle) * -steeringWheelAngleMultiplier),
                                                                Vector3.right);
                    break;
    ...
    void Update() {
        ...
        //Reversing Bool.
        if (brakeInput > .9f && transform.InverseTransformDirection(rigid.velocity).z < 1f && canGoReverseNow && automaticGear &&
            !semiAutomaticGear && !changingGear && direction != -1 && !RCC_InputManager.logitechHShifterUsed)
            StartCoroutine(ChangeGear(-1));
        else if (throttleInput < .1f && transform.InverseTransformDirection(rigid.velocity).z > -1f && direction == -1 && !changingGear &&
                 automaticGear && !semiAutomaticGear && !RCC_InputManager.logitechHShifterUsed)
            ...
            if (!externalController) {
                ...
                if (useSteeringSensitivity) {
                    ...
                    steerInput = Mathf.MoveTowards(steerInput, inputs.steerInput + counterSteerInput,
                        (Time.deltaTime * steeringSensitivityFactor * Mathf.Lerp(10f, 5f, steerAngle / orgSteerAngle)) *
                        (oppositeDirection ? 1f : 1f));
    ...
    private void SteeringAssistance() {
        ...
        if (useSteeringLimiter) {
            ...
            float maxSteerInput =
                Mathf.Clamp(1f - Mathf.Abs(sidewaysSlip), -1f, 1f); //	Subtract total average sideways slip from max steer input (1f).;
    ...
    void FixedUpdate() {
        ...
        if ((autoGenerateEngineRPMCurve) && oldEngineTorque != maxEngineTorque || oldMaxTorqueAtRPM != maxEngineTorqueAtRPM ||
        ...
        if (RCC_Settings.Instance.selectedBehaviorType != null && RCC_Settings.Instance.selectedBehaviorType.applyRelativeTorque) {
            ...
            if (isGrounded)
                rigid.AddRelativeTorque(Vector3.up * (((steerInput * throttleInput) * direction)) * Mathf.Lerp(1.5f, .5f, speed / 200f),
        ...
        // Applying downforce.
        rigid.AddForceAtPosition(-transform.up * (Mathf.Clamp(transform.InverseTransformDirection(rigid.velocity).z, 0f, 300f) * downForce),
            COM.transform.position, ForceMode.Force);
    ...
    private void Audio() {
        ...
        if (direction == 1)
            brakeSound.volume = Mathf.Lerp(0f, RCC_Settings.Instance.maxBrakeSoundVolume,
                Mathf.Clamp01((FrontLeftWheelCollider.wheelCollider.brakeTorque + FrontRightWheelCollider.wheelCollider.brakeTorque) /
                              (brakeTorque * 2f)) * Mathf.Lerp(0f, 1f, FrontLeftWheelCollider.wheelCollider.rpm / 50f));
        else
    ...
    private void AntiRollBars() {
        ...
        if (groundedFL)
            travelFL = (-FrontLeftWheelCollider.transform.InverseTransformPoint(FrontLeftWheelCollider.wheelHit.point).y -
                        FrontLeftWheelCollider.wheelCollider.radius) / FrontLeftWheelCollider.wheelCollider.suspensionDistance;
        ...
        if (groundedFR)
            travelFR = (-FrontRightWheelCollider.transform.InverseTransformPoint(FrontRightWheelCollider.wheelHit.point).y -
                        FrontRightWheelCollider.wheelCollider.radius) / FrontRightWheelCollider.wheelCollider.suspensionDistance;
        ...
        if (groundedRR)
            travelRR = (-RearRightWheelCollider.transform.InverseTransformPoint(RearRightWheelCollider.wheelHit.point).y -
                        RearRightWheelCollider.wheelCollider.radius) / RearRightWheelCollider.wheelCollider.suspensionDistance;
        ...
        if (currentGear == 0) {
            ...
            if (launched >= .25f)
                clutchInput = Mathf.Lerp(clutchInput,
                    (Mathf.Lerp(1f,
                        (Mathf.Lerp(clutchInertia, 0f, (wheelRPM / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity)) / gears[0].targetSpeedForNextGear)),
    ...
    public IEnumerator ChangeGear(int gear) {
        ...
        if (gearShiftingClips.Length > 0) {
            ...
            gearShiftingSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, gearSoundPosition, "Gear Shifting AudioSource", 1f, 5f,
                RCC_Settings.Instance.maxGearShiftingSoundVolume, gearShiftingClips[UnityEngine.Random.Range(0, gearShiftingClips.Length)], false,
                true, true);
    ...
    private void NOS() {
        ...
        if (!NOSSound)
            NOSSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, exhaustSoundPosition, "NOS Sound AudioSource", 5f, 10f, .5f,
                NOSClip, true, false, false);
    ...
    private void Turbo() {
        ...
        if (!turboSound) {
            ...
            turboSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, turboSoundPosition, "Turbo Sound AudioSource", .1f, .5f, 0,
                turboClip, true, true, false);
            NewHighPassFilter(turboSound, 10000f, 10);

        }

        turboBoost = Mathf.Lerp(turboBoost, Mathf.Clamp(Mathf.Pow(throttleInput, 10) * 30f + Mathf.Pow(engineRPM / maxEngineRPM, 10) * 30f, 0f, 30f),
            ...
            if (useCollisionAudio) {
                ...
                if (crashClips.Length > 0) {
                    ...
                    if (collision.GetContact(0).thisCollider.gameObject.transform != transform.parent) {
                        ...
                        crashSound = NewAudioSource(RCC_Settings.Instance.audioMixer, gameObject, "Crash Sound AudioSource", 5f, 20f,
                            RCC_Settings.Instance.maxCrashSoundVolume * (collision.impulse.magnitude / 10000f),
                            crashClips[UnityEngine.Random.Range(0, crashClips.Length)], false, true, true);
